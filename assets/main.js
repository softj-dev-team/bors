 (function () {
     if (cc.sys.isNative) {
            ...
            if( cc.sys.os == cc.sys.OS_ANDROID ){
                let ret = jsb.reflection.callStaticMethod( "com/zharev/ZharevUtil", "getVersion", "()Ljava/lang/String;");
                let jsonObj = JSON.parse(ret)
                let lastVersion = cc.sys.localStorage.getItem('HotUpdateInitVersion')
                if (lastVersion != jsonObj.shortVersion) {
                    cc.sys.localStorage.setItem('HotUpdateInitVersion',jsonObj.shortVersion)
                    jsb.fileUtils.removeDirectory(jsb.fileUtils.getWritablePath()+'game-remote-asset')
                    console.log('removeDirectory',jsb.fileUtils.getWritablePath()+'game-remote-asset',jsonObj.shortVersion,lastVersion)
                }
            }
         }
         ...
         var onStart = function () {
            if (window['sdkbox']) {
             ...
             cc.loader.downloader._subpackages = settings.subpackages;
              if (md5) {
                  cc.loader.downloader.handle = function (item, callback) {
                      var self = this;
             ...
             if (!false && !false) {
                 ...
                 if (cc.sys.isNative) {
                    cc.macro.DOWNLOAD_MAX_CONCURRENT = 1000;
                }
             }
